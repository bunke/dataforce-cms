...
if ($num) for ($i=0; $i<$num;$i++)
	...
	echo "<a href=".$pref_url.$FOLDER_IMAGES."/$TABLE_IMAGES.$row[crtdate].b.$row[format] target=_blank><img src=".$pref_url.$FOLDER_IMAGES."/$TABLE_IMAGES.$row[crtdate].s.jpg border=0 alt=\"$row[name_1]\" title=\"$row[name_1]\" hspace=\"3\" vspace=\"3\"></a><br>$row[name_1]<br>[ <a href=$act?id=$row[id]>".imedm."</a> <a  onClick=\"confirming('������� �������� $row[name_1]$row[zag_1]?','$act?id=$row[id]&del=1')\" href=#>".imdelm."</a> ]<hr size=1>";
	}
